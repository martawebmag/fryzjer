                ...
                <tbody>
                    @foreach ($appointments as $appointment)
                        <tr class="border-b hover:bg-gray-100">
                            ...
                            <td class="py-2 px-4">{{ $appointment->user->phone}}</td>
                            ...
                            <td class="py-2 px-4">
                                ...
                                <form action="{{ route('appointments.destroy', $appointment) }}" method="POST" class="inline-block ml-2">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="text-red-600 hover:text-red-900" onclick="return confirm('Czy na pewno chcesz usunąć tę wizytę?');">
                                        Usuń
                                    </button>
                                </form>
                            </td>
